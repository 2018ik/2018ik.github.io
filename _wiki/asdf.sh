...
if [ "$STEAM_DEBUGGER" == "gdb" ] || [ "$STEAM_DEBUGGER" == "cgdb" ]; then
    ...
    valgrind --tool=callgrind --instr-atstart=no "$STEAMROOT/$PLATFORM/$STEAMEXE" "$@"
elif [ "$STEAM_DEBUGGER" == "strace" ]; then
    strace -osteam.strace "$STEAMROOT/$PLATFORM/$STEAMEXE" "$@"
else
...
if [ $RESTART -ne 0 ] ; then
	...
	if [ $PLATFORM == "osx32" -a -f Info.plist ]; then
		STEAMAPP="$(cd "${0%/*}/../.." && echo $PWD)"
		OSXVER=`sw_vers -productVersion`
		case "$OSXVER" in
		10.5.* )
			exec open -n "$STEAMAPP"
			;;
		* )
			exec open -n "$STEAMAPP" --args "$@"
			;;
		esac
	else
		exec "$0" "$@"
